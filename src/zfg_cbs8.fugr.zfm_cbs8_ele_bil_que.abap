FUNCTION zfm_cbs8_ele_bil_que.
*"----------------------------------------------------------------------
*"*"本地接口：
*"  IMPORTING
*"     VALUE(INPUT) TYPE  ZSCBS8_EBQ
*"  EXPORTING
*"     VALUE(OUTPUT) TYPE  ZSCBS8_EBQ_OUT
...
*"----------------------------------------------------------------------
  ...
  CALL METHOD cl_cbs8->post2cbs8
    EXPORTING
      body        = body
      cbs8_suffix = '/openapi/account/openapi/v1/electronic-bill/query'
    IMPORTING
      output      = outputstr
      rtmsg       = DATA(msg)
      status      = DATA(status).
  ...
  ELSE.
    rtype = 'E'.
    rtmsg = |电子回单查询失败:[{ output-msg }],http响应状态码[{ status }],http响应消息[{ msg }]|.
  ENDIF.
  FREE cl_cbs8.
  zfmdatasave2 'R'.
ENDFUNCTION.
