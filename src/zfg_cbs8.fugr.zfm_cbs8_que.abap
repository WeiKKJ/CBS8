FUNCTION ZFM_CBS8_QUE.
*"----------------------------------------------------------------------
*"*"本地接口：
*"  IMPORTING
*"     VALUE(INPUT) TYPE  ZSCS8_Q
*"  EXPORTING
*"     VALUE(OUTPUT) TYPE  ZSCS8_Q_OUT
...
*"----------------------------------------------------------------------
  ...
  CALL METHOD cl_cbs8->post2cbs8
    EXPORTING
      body        = body
      cbs8_suffix = '/openapi/payment/openapi/v1/query'
    IMPORTING
      output      = outputstr
      rtmsg       = DATA(msg)
      status      = DATA(status).
  ...
  IF output-code = '0' AND msg NS '不一致!!!'.
    rtype = 'S'.
    rtmsg = '支付/代发/代扣列表查询成功'.
  ELSE.
    rtype = 'E'.
    rtmsg = |支付/代发/代扣列表查询失败:[{ output-msg }],http响应状态码[{ status }],http响应消息[{ msg }]|.
  ENDIF.
  FREE cl_cbs8.
  zfmdatasave2 'R'.
ENDFUNCTION.
