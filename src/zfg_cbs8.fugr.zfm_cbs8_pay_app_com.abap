FUNCTION zfm_cbs8_pay_app_com.
*"----------------------------------------------------------------------
*"*"本地接口：
*"  IMPORTING
*"     VALUE(INPUT) TYPE  ZLCBS8_PAC
*"  EXPORTING
*"     VALUE(OUTPUT) TYPE  ZSCBS8_PAC_OUT
...
*"----------------------------------------------------------------------
  ...
  CALL METHOD cl_cbs8->post2cbs8
    EXPORTING
      body        = body
      cbs8_suffix = '/openapi/payment/openapi/v1/payment-apply-common'
    IMPORTING
      output      = outputstr
      rtmsg       = DATA(msg)
      status      = DATA(status).
  ...
  IF output-code = '0' AND msg NS '不一致!!!'.
    rtype = 'S'.
    rtmsg = '支付CBS8响应正常,经办成功与否需要解析data节点'.
  ELSE.
    rtype = 'E'.
    rtmsg = |支付CBS8响应异常:[{ output-msg }],http响应状态码[{ status }],http响应消息[{ msg }]|.
  ENDIF.
  FREE cl_cbs8.
  zfmdatasave2 'R'.
ENDFUNCTION.
